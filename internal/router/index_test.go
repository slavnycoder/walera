...
func TestIndex_AddLookupRemove(t *testing.T) {
	...
	if got := idx.Lookup(key); got != nil {
		t.Errorf("Lookup on empty index: got %p; want nil", got)
	}

	idx.Add(key, sub)
	if got := idx.Lookup(key); got != sub {
		t.Errorf("Lookup after Add: got %p; want %p", got, sub)
	}
	...
	idx.Remove(key)
	if got := idx.Lookup(key); got != nil {
		t.Errorf("Lookup after Remove: got %p; want nil", got)
	}
	...
	for g := 0; g < G; g++ {
		...
		go func() {
			...
			for i := 0; i < N; i++ {
				...
				idx.Add(key, sub)
				if got := idx.Lookup(key); got != sub {
					t.Errorf("Lookup(%q) = %p; want %p", key, got, sub)
				}
				idx.Remove(key)
			}
		}()
